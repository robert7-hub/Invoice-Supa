...
const FormInput = ({ label, type = 'text', value, onChange, placeholder, multiline, rows = 3, theme }) => (
  <div className="space-y-1 sm:space-y-2">
    <label className={`text-[11px] sm:text-sm font-medium ${theme.textSecondary}`}>{label}</label>
    {multiline ? (
      <textarea value={value || ''} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows}
        className={`w-full px-3 py-2 sm:px-4 sm:py-3 border ${theme.inputBg} ${theme.inputBorder} rounded-lg sm:rounded-xl text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-slate-400 shadow-sm resize-none ${theme.textPrimary}`} />
    ) : (
      <input type={type} value={value || ''} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className={`w-full px-3 py-2 sm:px-4 sm:py-3 border ${theme.inputBg} ${theme.inputBorder} rounded-lg sm:rounded-xl text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-slate-400 shadow-sm ${theme.textPrimary}`} />
    )}
  </div>
);
...
const StatusBadge = ({ status, statusList = EVENT_STATUSES }) => {
  ...
  return <span className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[11px] sm:text-sm font-semibold ${s.color}`}>{s.label}</span>;
};
  ...
  return (
    <div className="relative">
      <button ref={btnRef} onClick={(e) => { e.stopPropagation(); setOpen(!open); }}
        className={`p-1 sm:p-2 rounded-lg sm:rounded-xl ${theme.buttonHover} transition-colors`}>
        <MoreVertical size={16} className={`${theme.textMuted} sm:hidden`} />
        <MoreVertical size={20} className={`${theme.textMuted} hidden sm:block`} />
      </button>
      {open && (
        <div ref={menuRef} style={menuStyle}
          className={`w-44 sm:w-52 ${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-xl shadow-xl py-1 sm:py-1.5 overflow-hidden`}>
          {items.map((item, idx) => (
            <button key={idx} onClick={(e) => { e.stopPropagation(); setOpen(false); item.onClick(); }}
              className={`w-full px-3 py-2 sm:px-4 sm:py-2.5 text-left text-xs sm:text-sm font-medium flex items-center gap-2 sm:gap-3 ${
...
const AssignStaffModal = ({ event, seData, theme, onClose, onAssign }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className={`${theme.cardBg} rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-lg mx-3 sm:mx-4 overflow-hidden max-h-[85vh] overflow-y-auto`} onClick={e => e.stopPropagation()} style={{ touchAction: 'auto' }}>
        <div className={`px-4 py-3 sm:px-6 sm:py-5 border-b ${theme.border} flex items-center justify-between`}>
          <div>
            <h3 className={`text-sm sm:text-lg font-bold ${theme.textPrimary}`}>Assign Staff</h3>
            <p className={`text-xs sm:text-sm ${theme.textMuted} mt-0.5`}>{event.title}</p>
          </div>
          <button onClick={onClose} className={`p-1.5 sm:p-2 rounded-lg sm:rounded-xl ${theme.buttonHover}`}><X size={16} className={`${theme.textMuted} sm:hidden`} /><X size={20} className={`${theme.textMuted} hidden sm:block`} /></button>
        </div>

        <div className="px-4 py-3 space-y-3 sm:px-6 sm:py-5 sm:space-y-5">
          {/* Staff */}
          <div className="space-y-2">
            ...
            <select value={staffId} onChange={e => {
              ...
              className={`w-full px-3 py-2 sm:px-4 sm:py-3 border ${theme.inputBorder} ${theme.inputBg} rounded-lg sm:rounded-xl text-sm sm:text-base ${theme.textPrimary}`}>
          ...
          {selectedStaff && selectedStaff.availability !== 'available' && (
            <div className="flex items-center gap-2 p-2.5 sm:p-3 bg-red-50 border border-red-200 rounded-lg sm:rounded-xl">
              ...
              <span className="text-xs sm:text-sm text-red-700 font-medium">{staffFullName(selectedStaff)} is marked as {selectedStaff.availability === 'on_leave' ? 'on leave' : 'unavailable'}</span>
            </div>
          )}
          {conflicts.length > 0 && (
            <div className="flex items-center gap-2 p-2.5 sm:p-3 bg-amber-50 border border-amber-200 rounded-lg sm:rounded-xl">
              <AlertTriangle size={18} className="text-amber-600" />
              <span className="text-xs sm:text-sm text-amber-700 font-medium">Conflict: already assigned to {conflicts.join(', ')} on this date</span>
            </div>
          )}
        </div>

        <div className={`px-4 py-3 sm:px-6 sm:py-4 border-t ${theme.border} flex gap-2 sm:gap-3`}>
          <button onClick={onClose}
            className={`flex-1 py-2 sm:py-3 border ${theme.border} rounded-lg sm:rounded-xl text-sm sm:text-base font-medium ${theme.textPrimary} ${theme.buttonHover}`}>Cancel</button>
          <button onClick={handleSubmit} disabled={!staffId || !finalRole}
            className={`flex-1 py-2 sm:py-3 ${theme.accent} rounded-lg sm:rounded-xl text-sm sm:text-base font-semibold ${theme.accentHover} disabled:opacity-40`}>Assign</button>
        </div>
      </div>
    </div>
  );
...
export default function StaffEventPage({ data, save, theme }) {
  ...
  const DashboardTab = () => {
    ...
    return (
      <div className="space-y-3 sm:space-y-6">
        <h2 className={`text-sm sm:text-xl font-bold ${theme.textPrimary}`}>Staff & Events Dashboard</h2>
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-2 sm:gap-4 md:grid-cols-3">
          {[
            ...
            <div key={stat.label} className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl p-2.5 sm:p-5 shadow-sm`}>
              <div className="flex items-center gap-1 sm:gap-2 mb-1.5 sm:mb-3">
                <stat.icon size={14} className={`${stat.color} sm:hidden`} />
                <stat.icon size={20} className={`${stat.color} hidden sm:block`} />
                <p className={`text-[8px] sm:text-sm font-semibold uppercase tracking-wide ${theme.textMuted} leading-tight`}>{stat.label}</p>
              </div>
              <p className={`text-lg sm:text-3xl font-bold ${theme.textPrimary}`}>{stat.value}</p>
            </div>
          ))}
        </div>
        ...
        {unassignedRoles.length > 0 && (
          <div className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl shadow-sm overflow-hidden`}>
            <div className={`px-3 py-2.5 sm:p-5 border-b ${theme.border}`}>
              <h3 className={`text-xs sm:text-lg font-semibold ${theme.textPrimary}`}>Missing Staff Assignments</h3>
            </div>
            <div className={`divide-y ${theme.border}`}>
              {unassignedRoles.map((item, idx) => (
                <div key={idx} className="px-3 py-2 sm:px-5 sm:py-4 flex items-center gap-2 sm:gap-3">
                  <AlertTriangle size={14} className="text-amber-500 sm:hidden" />
                  <AlertTriangle size={18} className="text-amber-500 hidden sm:block" />
                  <span className={`text-xs sm:text-base ${theme.textPrimary}`}><strong>{item.event}</strong> — Missing <strong>{item.role}</strong></span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl shadow-sm overflow-hidden`}>
          <div className={`px-3 py-2.5 sm:p-5 border-b ${theme.border}`}>
            <h3 className={`text-xs sm:text-lg font-semibold ${theme.textPrimary}`}>Upcoming Events</h3>
          </div>
          {upcomingEvents.length === 0 ? (
            <p className={`p-5 sm:p-8 text-center text-xs sm:text-base ${theme.textMuted}`}>No upcoming events</p>
          ) : (
            <div className={`divide-y ${theme.border}`}>
              {upcomingEvents.map(evt => {
                ...
                return (
                  <div key={evt.id} className={`px-3 py-2.5 sm:p-5 ${theme.cardHover}`}>
                    <div className="flex items-start justify-between">
                      <div>
                        <p className={`text-xs sm:text-base font-semibold ${theme.textPrimary}`}>{evt.title}</p>
                        <p className={`text-[10px] sm:text-sm ${theme.textMuted} mt-0.5`}>{client?.name || 'No client'} • {evt.venue || 'No venue'}</p>
                        <div className="flex items-center gap-2 sm:gap-4 mt-1 sm:mt-2">
                          <span className={`text-[10px] sm:text-sm ${theme.textMuted}`}><Calendar size={12} className="inline mr-1 sm:hidden" /><Calendar size={15} className="hidden sm:inline mr-1.5" />{evt.eventDate}</span>
                          <span className={`text-[10px] sm:text-sm ${theme.textMuted}`}><Clock size={12} className="inline mr-1 sm:hidden" /><Clock size={15} className="hidden sm:inline mr-1.5" />{evt.startTime || '—'} – {evt.endTime || '—'}</span>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1 sm:flex-row sm:items-center sm:gap-3">
                        {/* Calendar indicator */}
                        {assigned.length > 0 && (
                          <span className={`flex items-center gap-1 text-[10px] sm:text-xs font-semibold px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full ${
                        ...
                        {missingRoles.length === 0 && requiredRoles.length > 0 ? (
                          <span className="flex items-center gap-1 text-[10px] sm:text-sm font-semibold text-emerald-600"><CheckCircle size={13} className="sm:hidden" /><CheckCircle size={16} className="hidden sm:block" /> Fully Staffed</span>
                        ) : missingRoles.length > 0 ? (
                          <span className="flex items-center gap-1 text-[10px] sm:text-sm font-semibold text-amber-600"><AlertTriangle size={13} className="sm:hidden" /><AlertTriangle size={16} className="hidden sm:block" /> {missingRoles.length} missing</span>
                        ) : null}
  ...
  const StaffTab = () => {
      ...
      return (
        <div className="space-y-3 sm:space-y-5">
          <div className="flex items-center gap-3">
            ...
            <h2 className={`text-sm sm:text-xl font-bold ${theme.textPrimary}`}>{editingStaff ? 'Edit Staff' : 'New Staff Member'}</h2>
          </div>
          <div className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl p-3 sm:p-6 space-y-3 sm:space-y-5`}>
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2.5">
                {ROLES.map(role => (
                  <button key={role} onClick={() => toggleRole(role)}
                    className={`flex items-center gap-1.5 px-2.5 py-1 sm:gap-2 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl text-[11px] sm:text-sm font-semibold border transition-colors ${
            ...
            <div className="flex items-center justify-between">
              <span className={`text-xs sm:text-base font-medium ${theme.textPrimary}`}>Active</span>
              <button onClick={() => setForm({ ...form, active: !form.active })}
                className={`relative w-10 h-6 sm:w-14 sm:h-8 rounded-full ${form.active ? 'bg-emerald-500' : theme.toggleInactive}`}>
                <span className={`absolute top-1 sm:top-1.5 w-4 h-4 sm:w-5 sm:h-5 bg-white rounded-full transition-all ${form.active ? 'left-5 sm:left-7' : 'left-1 sm:left-1.5'}`} />
              </button>
            </div>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setView('list')} className={`flex-1 py-2 sm:py-3 border ${theme.border} rounded-lg sm:rounded-xl text-xs sm:text-base font-medium ${theme.textPrimary} ${theme.buttonHover}`}>Cancel</button>
            <button onClick={handleSave} className={`flex-1 py-2 sm:py-3 ${theme.accent} rounded-lg sm:rounded-xl text-xs sm:text-base font-semibold ${theme.accentHover}`}>Save</button>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-3 sm:space-y-5">
        <div className="flex items-center justify-between">
          <h2 className={`text-sm sm:text-xl font-bold ${theme.textPrimary}`}>Staff ({seData.staff.length})</h2>
          <button onClick={startNew} className={`flex items-center gap-1.5 px-3 py-1.5 sm:gap-2 sm:px-5 sm:py-2.5 ${theme.accent} rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold ${theme.accentHover}`}><Plus size={14} className="sm:hidden" /><Plus size={18} className="hidden sm:block" /> Add Staff</button>
        </div>

        <div className="relative">
          <Search size={14} className={`absolute left-3 top-1/2 -translate-y-1/2 ${theme.textMuted} sm:hidden`} />
          <Search size={18} className={`absolute left-4 top-1/2 -translate-y-1/2 ${theme.textMuted} hidden sm:block`} />
          <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search staff..."
            className={`w-full pl-9 pr-3 py-2 sm:pl-12 sm:pr-4 sm:py-3 border ${theme.inputBorder} ${theme.inputBg} rounded-lg sm:rounded-xl text-sm sm:text-base ${theme.textPrimary}`} />
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2.5">
          {[
            ...
            <button key={f.value} onClick={() => setStaffFilter(f.value)}
              className={`px-2.5 py-1 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl text-[11px] sm:text-sm font-medium ${staffFilter === f.value ? theme.accent : `border ${theme.border} ${theme.textSecondary} ${theme.buttonHover}`}`}>
              {f.label}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2 sm:space-y-3">
            {filtered.map(s => {
              ...
              return (
              <div key={s.id} className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl shadow-sm p-3 sm:p-5 ${theme.cardHover} cursor-pointer`} onClick={() => startEdit(s)}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className={`w-8 h-8 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-xs sm:text-base font-bold ${s.active ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
                      {staffInitials(s)}
                    </div>
                    <div>
                      <p className={`text-xs sm:text-base font-semibold ${theme.textPrimary}`}>{staffFullName(s)} {!s.active && <span className="text-[10px] sm:text-sm text-gray-400">(Inactive)</span>}</p>
                      <div className="flex items-center gap-2 mt-1">
                        {s.roles.map(r => (
                          <span key={r} className={`text-[10px] sm:text-xs font-medium px-1.5 py-0.5 rounded ${theme.subtleBg} ${theme.textMuted}`}>{r}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-4">
                    {assignments.length > 0 && (
                      <span className="text-[10px] sm:text-xs font-semibold px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-indigo-100 text-indigo-700">{assignments.length} job{assignments.length > 1 ? 's' : ''}</span>
                    )}
                    {s.rate > 0 && <span className={`text-xs sm:text-base font-medium ${theme.textMuted}`}>{formatCurrency(s.rate)}</span>}
                    <button onClick={e => { e.stopPropagation(); handleDelete(s.id); }}
                      className="p-1 sm:p-2 rounded-lg hover:bg-red-500/10"><Trash2 size={14} className="text-red-400 sm:hidden" /><Trash2 size={18} className="text-red-400 hidden sm:block" /></button>
                  </div>
                </div>
  ...
  const EventsTab = () => {
    ...
    if (view === 'form') {
      return (
        <div className="space-y-3 sm:space-y-5">
          <div className="flex items-center gap-3">
            <button onClick={() => setView('list')} className={`p-2 ${theme.buttonHover} rounded-xl`}><ChevronLeft size={22} className={theme.textPrimary} /></button>
            <h2 className={`text-sm sm:text-xl font-bold ${theme.textPrimary}`}>{editingEvent ? 'Edit Event' : 'New Event'}</h2>
          </div>
          <div className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl p-3 sm:p-6 space-y-3 sm:space-y-5`}>
          ...
          <div className="flex gap-3">
            <button onClick={() => setView('list')} className={`flex-1 py-2 sm:py-3 border ${theme.border} rounded-lg sm:rounded-xl text-xs sm:text-base font-medium ${theme.textPrimary} ${theme.buttonHover}`}>Cancel</button>
            <button onClick={handleSave} className={`flex-1 py-2 sm:py-3 ${theme.accent} rounded-lg sm:rounded-xl text-xs sm:text-base font-semibold ${theme.accentHover}`}>Save Event</button>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-3 sm:space-y-5">
        <div className="flex items-center justify-between">
          <h2 className={`text-sm sm:text-xl font-bold ${theme.textPrimary}`}>Events ({seData.events.length})</h2>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg sm:rounded-2xl p-3 sm:p-5">
            ...
            <div className="space-y-1.5 sm:space-y-2.5">
              {convertableEstimates.map(est => {
                const client = getClient(est.clientId);
                return (
                  <div key={est.id} className="flex items-center justify-between bg-white rounded-lg sm:rounded-xl px-2.5 py-2 sm:px-4 sm:py-3 border border-blue-100">
                    <div className="min-w-0 flex-1 mr-2">
                      <p className="text-[11px] sm:text-base font-medium text-blue-900 truncate">{est.number} — {client?.name || 'No client'}</p>
                      <p className="text-[10px] sm:text-sm text-blue-600">{est.date}</p>
                    </div>
                    <button onClick={() => convertEstimate(est)}
                      className="px-2.5 py-1 sm:px-4 sm:py-2 bg-blue-600 text-white rounded-lg sm:rounded-xl text-[10px] sm:text-sm font-semibold hover:bg-blue-700 shrink-0">
                      Convert to Event
                    </button>
                  </div>
                );
        ...
        <div className="flex flex-wrap gap-1 sm:gap-2.5">
          {[{ value: 'all', label: 'All' }, ...EVENT_STATUSES].map(f => (
            <button key={f.value} onClick={() => setFilter(f.value)}
              className={`px-2 py-1 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-sm font-medium ${filter === f.value ? theme.accent : `border ${theme.border} ${theme.textSecondary} ${theme.buttonHover}`}`}>
              {f.label}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2.5 sm:gap-4">
            {filtered.sort((a, b) => b.eventDate.localeCompare(a.eventDate)).map(evt => {
              ...
              return (
                <div key={evt.id} className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl shadow-sm overflow-hidden flex flex-col`}>
                  {/* Card header */}
                  <div className="px-3 py-2.5 sm:p-5 sm:pb-3">
                    <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 mt-1.5 sm:mt-3">
                      <span className={`flex items-center gap-1 text-[10px] sm:text-sm ${theme.textMuted}`}><Calendar size={11} className="sm:hidden" /><Calendar size={14} className="hidden sm:block" />{evt.eventDate}</span>
                      {evt.startTime && <span className={`flex items-center gap-1 text-[10px] sm:text-sm ${theme.textMuted}`}><Clock size={11} className="sm:hidden" /><Clock size={14} className="hidden sm:block" />{evt.startTime}–{evt.endTime || '?'}</span>}
                    </div>

                    {/* Status + staffing + calendar badges */}
                    <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap mt-1.5 sm:mt-3">
                      <StatusBadge status={evt.status} />
                      {required.length > 0 && (
                        missing.length === 0 ? (
                          <span className="flex items-center gap-0.5 text-[10px] sm:text-xs font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full"><CheckCircle size={11} className="sm:hidden" /><CheckCircle size={13} className="hidden sm:block" /> Fully Staffed</span>
                        ) : (
                          <span className="flex items-center gap-0.5 text-[10px] sm:text-xs font-semibold text-amber-600 bg-amber-50 px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full"><AlertTriangle size={11} className="sm:hidden" /><AlertTriangle size={13} className="hidden sm:block" /> {missing.join(', ')}</span>
                        )
                      )}
                      {assigned.length > 0 && (
                        <span className={`flex items-center gap-0.5 text-[10px] sm:text-xs font-semibold px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full ${
                          allCalendarBooked ? 'bg-emerald-100 text-emerald-700' :
                          someCalendarBooked ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-500'
                        }`}>
                          <Calendar size={11} className="sm:hidden" />
                  ...
                  {assigned.length > 0 && (
                    <div className={`px-3 py-2 sm:px-5 sm:py-3 border-t ${theme.border}`}>
                      <p className={`text-[9px] sm:text-xs font-semibold uppercase tracking-wider ${theme.textMuted} mb-1 sm:mb-2`}>Assigned Staff</p>
                      <div className="flex flex-wrap gap-1.5 sm:gap-2">
                        {assigned.map(a => {
                          const staffMember = seData.staff.find(s => s.id === a.staffId);
                          return (
                            <span key={a.id} className={`inline-flex items-center gap-1 text-[10px] sm:text-sm font-medium px-2 py-0.5 sm:px-3 sm:py-1.5 rounded-full ${theme.subtleBg} ${theme.textPrimary} border ${theme.border}`}>
                  ...
                  {/* Quick-action footer */}
                  <div className={`mt-auto px-3 py-2 sm:px-5 sm:py-3 border-t ${theme.border} flex items-center justify-between`}>
                    <button onClick={() => setAssignModalEvent(evt)}
                      className={`flex items-center gap-1 text-[10px] sm:text-sm font-medium text-blue-600 hover:text-blue-700`}>
                      <Plus size={12} className="sm:hidden" /><Plus size={15} className="hidden sm:block" /> Assign Staff
                    </button>
                    {assigned.length > 0 && (
                      <div className="flex items-center gap-1">
                        {assigned.filter(a => !a.calendarCreated).length > 0 && (
                          <button onClick={() => {
                            ...
                            className="flex items-center gap-1 text-[10px] sm:text-sm font-medium text-emerald-600 hover:text-emerald-700">
                            <Calendar size={12} className="sm:hidden" /><Calendar size={15} className="hidden sm:block" /> Book All
                          </button>
                        )}
                      </div>
                    )}
  ...
  const PaymentsTab = () => {
    ...
    return (
      <div className="space-y-3 sm:space-y-5">
        <h2 className={`text-sm sm:text-xl font-bold ${theme.textPrimary}`}>Staff Payments</h2>

        {/* Summary */}
        <div className="grid grid-cols-2 gap-2 sm:gap-4">
          <div className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl p-3 sm:p-5`}>
            <p className={`text-[10px] sm:text-sm font-semibold uppercase tracking-wide text-red-500`}>Unpaid</p>
            <p className={`text-base sm:text-2xl font-bold text-red-600 mt-0.5 sm:mt-1`}>{formatCurrency(totalUnpaid)}</p>
          </div>
          <div className={`${theme.cardBg} border ${theme.border} rounded-lg sm:rounded-2xl p-3 sm:p-5`}>
            <p className={`text-[10px] sm:text-sm font-semibold uppercase tracking-wide text-emerald-500`}>Paid</p>
            <p className={`text-base sm:text-2xl font-bold text-emerald-600 mt-0.5 sm:mt-1`}>{formatCurrency(totalPaid)}</p>
          </div>
        </div>
        ...
        <div className="flex gap-1 sm:gap-2.5">
          {['all', 'unpaid', 'paid'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-2 py-1 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-sm font-medium capitalize ${filter === f ? theme.accent : `border ${theme.border} ${theme.textSecondary} ${theme.buttonHover}`}`}>
              {f}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {filtered.map(pay => {
              ...
              return (
                <div key={pay.id} className={`${theme.cardBg} border ${theme.border} rounded-xl p-4 shadow-sm`}>
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold ${theme.textPrimary}`}>{staffFullName(staff)}</p>
                      <p className={`text-xs ${theme.textMuted} mt-0.5`}>{evt?.title || 'Unknown event'} • {evt?.eventDate || '—'}</p>
                      {pay.paidDate && <p className="text-xs text-emerald-500 mt-0.5">Paid on {pay.paidDate}</p>}
                    </div>
                    <div className="flex items-center gap-2 shrink-0 ml-3">
                      <p className={`text-sm font-bold ${pay.status === 'paid' ? 'text-emerald-600' : 'text-red-600'}`}>{formatCurrency(pay.amount)}</p>
                      {pay.status === 'unpaid' ? (
                      ...
                      <button onClick={() => deletePayment(pay.id)}
                        className="p-1.5 rounded-lg hover:bg-red-500/10"><Trash2 size={16} className="text-red-400" /></button>
                    </div>
                  </div>
                </div>
              );
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Sub-tab bar */}
      <div className={`px-3 py-2 sm:px-5 sm:py-4 border-b ${theme.border} flex gap-1.5 sm:gap-3 overflow-x-auto`}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setSubTab(t.id)}
            className={`px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-base font-medium whitespace-nowrap ${
              subTab === t.id ? theme.accent : `${theme.textSecondary} ${theme.buttonHover}`
            }`}
            style={subTab === t.id ? { textShadow: '0 1px 3px rgba(0,0,0,0.3)' } : undefined}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {/* Content */}
      <div className="flex-1 overflow-auto px-3 py-2.5 phone-dock-scroll-space sm:p-5 lg:p-6 lg:pb-6">
        {renderSubTab()}
      </div>
    </div>
  );
